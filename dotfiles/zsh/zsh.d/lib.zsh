...
# -----------------------------------------------------------------------------
# mise (言語バージョン管理) を有効にする
#
# この処理は約19msかかるので、__run_later で「プロンプトを出した後」に回している。
# ただしそれだと有効になるまで node や go が見つからない一瞬ができる。
# そこで .zshrc 側で shims (どのバージョンを使うか判断してくれる中継役) を
# 先に PATH へ入れてある。だから待っている間も node は正しく動く。
#
# activate の出力には解決済みの PATH がそのまま埋め込まれるため、
# 他のコマンドと違って結果をキャッシュすることはできない。
# -----------------------------------------------------------------------------
__setup_mise() {
  local mise_path=${commands[mise]:-$HOME/.local/bin/mise}
  [[ -x $mise_path ]] || return 0

  eval "$($mise_path activate zsh)"

  # mise は既定で「プロンプトを出すたび」に設定を確認しにいく (約11ms)。
  # 実際にバージョンが切り替わるのはディレクトリを移動したときなので、
  # プロンプト側のフックを外して移動時だけにする。
  #
  # 副作用: ディレクトリを移動せずに mise.toml を書き換えた場合、
  #         その場では反映されない。cd するか新しいターミナルを開けば反映される
  add-zsh-hook -d precmd _mise_hook_precmd

  # mise は PATH を文字列でまとめて入れ直すため重複が残る。配列に入れ直して消す
  path=($path)
}
